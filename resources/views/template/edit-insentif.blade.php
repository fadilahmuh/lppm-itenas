    ...
    <div class="intro-y col-span-12 lg:col-span-8">
        ...
        <div class="box p-5" id="input">
            <div class="">
                ...
                <select name="jenis_insentif" class="select2 input w-full border mt-2 select-insentif" style="width: 100%" data-placeholder="--Pilih Jenis Hibah--" data-list="{{ route('get_insentif') }}">
                    <option selected="selected" value="{{$insentif->jenis_insentif->id}}">{{$insentif->jenis_insentif->nama}}</option>
                </select>
            </div>
            <div class="mt-3">
                <label>Judul Publikasi</label>
                <input name="judul_publikasi" type="text" class="input w-full border mt-2" placeholder="--Judul Publikasi--" value="{{$insentif->judul}}">
            </div>
            <div class="mt-3">
                ...
                <input name="dosen_ketua" type="hidden" class="input w-full border mt-2" value="{{$insentif->dosen_ketua->id}}">
                @endif 
            </div>
            <div class="mt-3">
                <label class="mb-2">Penulis Anggota</label>
                <select name="dosen_anggota[]" class="select2 input w-full border mt-2 select-dosen" multiple data-placeholder="--Pilih penulis anggota--" data-list="{{ route('get_dosen') }}">
            ...
            <div class="mt-3">
                ...
                <select name="jenis_publikasi" class="select2 input w-full border mt-2 select-pub" style="width: 100%" data-placeholder="--Pilih jenis publikasi--" data-list="{{ route('get_pub') }}">
                    <option selected="selected" value="{{$insentif->jenis_publikasi->id}}">{{$insentif->jenis_publikasi->nama}}</option>
                </select>
            </div>
            <div class="mt-3">
                <label>Jurnal</label>
                <input name="jurnal" type="text" class="input w-full border mt-2" placeholder="--Nama Jurnal--" value="{{$insentif->jurnal}}">
            </div>
                ...
                <div class="grid grid-cols-12 gap-2">
                    <div class="col-span-4">
                        ...
                        <div class="relative mt-2">
                            ...
                            <input name="tahun" type="text" class="bs-yearpicker input pl-12 w-full border col-span-4" value="{{$insentif->tahun}}" placeholder="--Pilih Tahun--" autocomplete="off">
                        </div>
                    </div>
                </div>
            </div>
            <div class="mt-3">
                ...
                <div class="relative mt-2">
                    ...
                    <input name="jumlah" type="text" class="input rupiah pl-12 w-full border col-span-4" placeholder="--Jumlah Dana Hibah--" autocomplete="off" value="{{$insentif->jumlah}}">
                </div>
            </div>
            ...
            <div class="mt-4">
                ...
                <div class="flex flex-col sm:flex-row mt-2 status-group">
                    <div class="flex items-center text-gray-700 mr-4">
                        <input type="radio" class="input border mr-2" name="status" value="1" @auth('dosen') disabled @endauth @if($insentif->status == 1) checked @endif>
                        <label class="cursor-pointer select-none" for="status-posted">Posted</label>
                    </div>
                    <div class="flex items-center text-gray-700 mr-2">
                        <input type="radio" class="input border mr-2" name="status" value="0" @if($insentif->status == 0) checked @elseif(Auth::guard('dosen')->check()) checked @endif >
