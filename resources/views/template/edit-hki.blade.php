    ...
    <div class="intro-y col-span-12 lg:col-span-8">
        ...
        <div class="box p-5" id="input">
            <div>
                ...
                <input type="text" name="nama_hki" class="input w-full border mt-2" placeholder="--Nama HKI--" value="{{$hki->judul}}">
            </div>
            <div class="mt-3">
                <label>Jenis HKI</label>
                <input type="text" name="jenis_hki" class="input w-full border mt-2" placeholder="--Jenis HKI--" value="{{$hki->jenis_hki}}">
            </div>
            <div class="mt-3">
                <label>Dosen Ketua</label>
                @if(Auth::guard('pegawai')->check())
                <select id="pen" name="dosen_ketua" class="select2 input w-full border mt-2 select-dosen" data-placeholder="--Pilih dosen ketua--" data-list="{{ route('get_dosen') }}">
                ...
                <input name="dosen_ketua" type="hidden" class="input w-full border mt-2" value="{{$hki->dosen_ketua->id}}">
                @endif 
            </div>
                ...
                <div class="grid grid-cols-12 gap-2">
                    <div class="col-span-4">
                        ...
                        <div class="relative mt-2">
                            ...
                            <input name="tahun" type="text" class="bs-yearpicker input pl-12 w-full border col-span-4" value="{{$hki->tahun}}" placeholder="--Pilih Tahun--" autocomplete="off">
                        </div>
                    </div>
                </div>
            </div>
            <div class="mt-3">
                ...
                <div class="relative mt-2">
                    ...
                    <input name="jumlah" type="text" class="input rupiah pl-12 w-full border col-span-4" placeholder="--Jumlah Dana Hibah--" autocomplete="off" value="{{$hki->jumlah}}">
                </div>
            </div>
            ...
            <div class="mt-4">
                ...
                <div class="flex flex-col sm:flex-row mt-2 status-group">
                    <div class="flex items-center text-gray-700 mr-4">
                        <input type="radio" class="input border mr-2" name="status" value="1" @auth('dosen') disabled @endauth @if($hki->status == 1) checked @endif>
                        <label class="cursor-pointer select-none" for="status-posted">Posted</label>
                    </div>
                    <div class="flex items-center text-gray-700 mr-2">
                        <input type="radio" class="input border mr-2" name="status" value="0" @if($hki->status == 0) checked @elseif(Auth::guard('dosen')->check()) checked @endif >
